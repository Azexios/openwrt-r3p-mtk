...
3. Скачать конфиг (для сборки OpenWrt с правильным .vermagiс), ниже пример для OpenWrt 22.03.5 и устройств с CPU mt7621:

    ```bash
    wget -O .config https://downloads.openwrt.org/releases/22.03.5/targets/ramips/mt7621/config.buildinfo
    make defconfig
    ```
...
Не обязательно - открыть profiles.json (для mt7621 и OpenWrt 22.03.5 - https://downloads.openwrt.org/releases/22.03.5/targets/ramips/mt7621/profiles.json), найти своё устройство и посмотреть какие пакеты (device_packages, кроме Wi-Fi драйверов) нужно дополнительно включить в прошивку.
