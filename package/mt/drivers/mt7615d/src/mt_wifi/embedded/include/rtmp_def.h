...
#if defined(MT7615) || defined(MT7622) || defined(P18) || defined(MT7663)
#define TX_DMA_1ST_BUFFER_SIZE                                                 \
	128 /* only the 1st physical buffer is pre-allocated */
#else
/* TXINFO_SIZE + TXWI_SIZE + 802.11 Header Size + AMSDU sub frame header */
#define TX_DMA_1ST_BUFFER_SIZE                                                 \
	96 /* only the 1st physical buffer is pre-allocated */
...
#define MAX_LEN_OF_MULTICAST_FILTER_HASH_TABLE                                 \
	((MAX_LEN_OF_MULTICAST_FILTER_TABLE)*2)
...
#define fRTMP_ADAPTER_START_UP                                                 \
	0x10000000 /*Devive already initialized and enabled Tx/Rx. */
/* #ifdef CONFIG_PM */
...
enum WIFI_MODE {
	...
	WMODE_AC = 1 << 5,
	WMODE_COMP =
		6, /* total types of supported wireless mode, add this value once yow add new type */
};
#endif
...
#define fRTMP_PS_TOGGLE_L1                                                     \
	0x00000010 /* Use Toggle L1 mechanism for rt28xx PCIe */
...
#define ALLN_SETPROTECT                                                        \
	(GR40SETPROTECT | GF20SETPROTECT | MM40SETPROTECT | MM20SETPROTECT)

/*
	AP's client table operation status flags
*/
#define fCLIENT_STATUS_WMM_CAPABLE                                             \
	0x00000001 /* CLIENT can parse QOS DATA frame */
#define fCLIENT_STATUS_AGGREGATION_CAPABLE                                     \
	0x00000002 /* CLIENT can receive Ralink's proprietary TX aggregation frame */
#define fCLIENT_STATUS_PIGGYBACK_CAPABLE                                       \
	0x00000004 /* CLIENT support piggy-back */
...
#endif /* DOT11N_DRAFT3 */
#define fCLIENT_STATUS_SOFTWARE_ENCRYPT                                        \
	0x00002000 /* Indicate the client encrypt/decrypt by software */
...
#define HT_ATHEROS                                                             \
	8 /* rt2860c has problem with atheros chip. we need to turn on RTS/CTS . */
#define HT_FORCERTSCTS                                                         \
	9 /* Force turn on RTS/CTS first. then go to evaluate if this force RTS is necessary. */
...
#endif /* WDS_SUPPORT */
#define WDS_PAIRWISE_KEY_OFFSET                                                \
	60 /* WDS links uses pairwise key#60 ~ 63 in ASIC pairwise key table */
...
#define WDEV_NUM_MAX                                                           \
	(HW_BEACON_MAX_NUM + MAX_WDS_ENTRY + MAX_APCLI_NUM + MAX_P2P_NUM +     \
	 MAX_MESH_NUM + MONITOR_MAX_DEV_NUM)
...
#define BSSINFO_NUM_MAX(_pChipCap)                                             \
	(HW_BSSID_MAX + EXTEND_MBSS_MAC_MAX + MAX_WDS_ENTRY +                  \
	 GET_MAX_REPEATER_ENTRY_NUM(_pChipCap))
#else
#define BSSINFO_NUM_MAX(_pChipCap)                                             \
	(HW_BSSID_MAX + EXTEND_MBSS_MAC_MAX + MAX_WDS_ENTRY)
...
#define MBSS_MR_APIDX_SANITY_CHECK(__pAd, apidx)                               \
	{                                                                      \
		if ((apidx >= MAX_MBSSID_NUM(__pAd)) ||                        \
			...
			MTWF_LOG(DBG_CAT_AP, CATAP_MBSS, DBG_LVL_ERROR,        \
				 ("%s> Error! apidx = %d > MAX_MBSSID_NUM!\n", \
				  __func__, apidx));                           \
			apidx = MAIN_MBSSID;                                   \
		}                                                              \
	}

#define VALID_MBSS(_pAd, _apidx)                                               \
	((_apidx < MAX_MBSSID_NUM(_pAd)) && (_apidx < HW_BEACON_MAX_NUM))
...
#define GET_PMF_GroupKey_WCID(__pAd, __wcid, __bssidx)                         \
	{                                                                      \
		__wcid = LAST_SPECIFIC_WCID(__pAd) -                           \
			 (MAX_MBSSID_NUM(__pAd) * 2) + __bssidx;               \
	}
...
#define MAX_LEN_OF_SUPPORTED_RATES                                             \
	MAX_LENGTH_OF_SUPPORT_RATES /* 1, 2, 5.5, 11, 6, 9, 12, 18, 24, 36, 48, 54 */
#define MAX_LEN_OF_SUPPORTED_CHL 64
#define MAX_NUM_OF_REGULATORY_CLASS 16
#define MAX_LEN_OF_KEY 32 /* 32 octets == 256 bits, Redefine for WPA */
/* #define MAX_NUM_OF_CHANNELS             MAX_NUM_OF_CHS */ /* 14 channels @2.4G +  12@UNII + 4 @MMAC + 11 @HiperLAN2 + 7 @Japan + 1 as NULL termination */
#define MAX_NUM_OF_11JCHANNELS                                                 \
	20 /* 14 channels @2.4G +  12@UNII + 4 @MMAC + 11 @HiperLAN2 + 7 @Japan + 1 as NULL termination */
...
#define MAX_LEN_OF_BA_REC_TABLE                                                \
	((NUM_OF_TID * MAX_LEN_OF_MAC_TABLE) /                                 \
	 2) /*   (NUM_OF_TID*MAX_AID_BA + 32)        //Block ACK recipient */
#define MAX_LEN_OF_BA_ORI_TABLE                                                \
	((NUM_OF_TID * MAX_LEN_OF_MAC_TABLE) /                                 \
	 2) /*   (NUM_OF_TID*MAX_AID_BA + 32)   // Block ACK originator */
...
#define MLME_INVALID_SECURITY_POLICY                                           \
...
#define IE_SECONDARY_CH_OFFSET                                                 \
	62 /* 802.11n D3.03        Secondary Channel Offset element */
#define IE_WAPI                                                                \
	68 /* WAPI information element. Same as Bss Ac Access Dealy Element. */
#define IE_TIME_ADVERTISEMENT 69 /* 802.11p */
#define IE_RM_ENABLE 70 /* 802.11mc D4.0 */
#define IE_2040_BSS_COEXIST 72 /* 802.11n D3.0.3 */
#define IE_2040_BSS_INTOLERANT_REPORT 73 /* 802.11n D3.03 */
#define IE_OVERLAPBSS_SCAN_PARM 74 /* 802.11n D3.03 */
#define IE_CHANNEL_USAGE                                                       \
	97 /* Cisco advertises suggested channel using this IE. */
...
#define IE_LAST_BCN_REPORT_INDICATION_REQUEST                                  \
	164 /*Last Beacon Report Indication Request*/
...
#define IE_AIRONET_CCKMREASSOC                                                 \
	156 /* CCX ID 9CH for CCKM Reassociation Request element */
...
#define MAX_IEEE_STD_CATE                                                      \
	21 /* Indicate the maximum category code defined in IEEE-802.11-Std */
...
/* #ifdef WFD_SUPPORT */
#define CATEGORY_VENDOR_SPECIFIC_WFD                                           \
	127 /* CFG_TODO CATEGORY_VENDOR, WHY ONLY WFD ? */
/* #endif */ /* WFD_SUPPORT */
...
*/
#define SYNC_IDLE                                                              \
	0 /* merge NO_BSS,IBSS_IDLE,IBSS_ACTIVE and BSS in to 1 state */
...
#define APCLI_CTRL_DISCONNECTED                                                \
	0 /* merge NO_BSS,IBSS_IDLE,IBSS_ACTIVE and BSS in to 1 state */
...
#define MCS_SHORTP_RATE_1                                                      \
	4 /* long preamble CCK 1Mbps. short is forbidden in 1Mbps */
...
#define BW_10                                                                  \
	BAND_WIDTH_10 /* 802.11j has 10MHz. This definition is for internal usage. doesn't fill in the IE or other field. */
...
#define EVENT_ASSOCIATED                                                       \
	1 /* Log = "hh:mm:ss   STA 00:01:02:03:04:05 associated" */
#define EVENT_DISASSOCIATED                                                    \
	2 /* Log = "hh:mm:ss   STA 00:01:02:03:04:05 left this BSS" */
#define EVENT_AGED_OUT                                                         \
	3 /* Log = "hh:mm:ss   STA 00:01:02:03:04:05 was aged-out and removed from this BSS" */
...
#define RD_SILENCE_MODE                                                        \
	2 /* After channel switch, need to be silence a while to ensure radar not found */
...
enum { ENTRY_STATE_NONE, ENTRY_STATE_SYNC };
...
#ifdef CLIENT_WDS
#define IS_ENTRY_CLIWDS(_x)                                                    \
	CLIENT_STATUS_TEST_FLAG((_x), fCLIENT_STATUS_CLI_WDS)
#endif /* CLIENT_WDS */
...
#ifdef CLIENT_WDS
#define SET_ENTRY_CLIWDS(_x)                                                   \
	CLIENT_STATUS_SET_FLAG((_x), fCLIENT_STATUS_CLI_WDS)
#endif /* CLIENT_WDS */
/* CFG_TODO */
...
#define IW_SYS_EVENT_TYPE_NUM                                                  \
	(IW_SYS_EVENT_FLAG_END - IW_SYS_EVENT_FLAG_START + 1)
...
#define IW_SPOOF_EVENT_TYPE_NUM                                                \
	(IW_SPOOF_EVENT_FLAG_END - IW_SPOOF_EVENT_FLAG_START + 1)
...
#define IW_FLOOD_EVENT_TYPE_NUM                                                \
	(IW_FLOOD_EVENT_FLAG_END - IW_FLOOD_EVENT_FLAG_START + 1)
...
#define IW_WSC_EVENT_TYPE_NUM                                                  \
	(IW_WSC_EVENT_FLAG_END - IW_WSC_EVENT_FLAG_START + 1)
...
enum { HCCA_TO_EDCA, EDCA_TO_HCCA = 0x55 };
...
#define RT_80211_TO_8023_PACKET(__pAd, __VLAN_VID, __VLAN_Priority, __pRxBlk,  \
				__pHeader802_3, _wdev_idx, __TPID)             \
	{                                                                      \
		wlan_802_11_to_802_3_packet(                                   \
			get_netdev_from_bssid(__pAd, _wdev_idx),               \
			__pRxBlk->OpMode, __VLAN_VID, __VLAN_Priority,         \
			__pRxBlk->pRxPacket, __pRxBlk->pData,                  \
			__pRxBlk->DataSize, __pHeader802_3, __TPID);           \
	}
...
#define RTMP_L2_FRAME_TX_ACTION(__pAd, __ApIdx, __FrameBuf, __FrameLen)        \
	RTMPL2FrameTxAction(__pAd, get_netdev_from_bssid(__pAd, __ApIdx),      \
			    announce_802_3_packet, __ApIdx, __FrameBuf,        \
			    __FrameLen, __pAd->OpMode)
...
#define RTMPSendWirelessEvent(__pAd, __Event_flag, __pAddr, _wdev_idx, __Rssi) \
	RtmpOsSendWirelessEvent(__pAd, __Event_flag, __pAddr, _wdev_idx,       \
				__Rssi, RtmpDrvSendWirelessEvent);
#else
...
#define RTMP_OS_TASK_INIT(__pTask, __pTaskName, __pAd)                         \
	RtmpOSTaskInit(__pTask, __pTaskName, __pAd, &(__pAd)->RscTaskMemList,  \
		       &(__pAd)->RscSemMemList);
