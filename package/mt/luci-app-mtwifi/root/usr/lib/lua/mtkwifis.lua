...
if not mtkwifis.exists("/etc/wireless/mt7603/") then -- MT7615
	...
	do
		os.execute("cat /tmp/dhcp.leases | grep -i '"..mac.."' | awk '{print $4}' | grep '.*' >> /tmp/mtk/host || echo - >> /tmp/mtk/host")
		os.execute("cat /tmp/dhcp.leases | grep -i '"..mac.."' | awk '{print $3}' | grep '.*' >> /tmp/mtk/ip || echo - >> /tmp/mtk/ip")
	end

	Host = mtkwifis.read_pipe("cat /tmp/mtk/host 2>/dev/null") or "?"
	IP = mtkwifis.read_pipe("cat /tmp/mtk/ip 2>/dev/null") or "?"
	os.execute("rm -rf /tmp/mtk/host /tmp/mtk/ip")
elseif mtkwifis.exists("/etc/wireless/mt7603/") and mtkwifis.exists("/etc/wireless/mt7612/") then -- MT7603+MT7612
	...
	do
		os.execute("cat /tmp/dhcp.leases | grep -i '"..mac2.."' | awk '{print $4}' | grep '.*' >> /tmp/mtk/host2 || echo - >> /tmp/mtk/host2")
		os.execute("cat /tmp/dhcp.leases | grep -i '"..mac2.."' | awk '{print $3}' | grep '.*' >> /tmp/mtk/ip2 || echo - >> /tmp/mtk/ip2")
	end

	Host = mtkwifis.read_pipe("cat /tmp/mtk/host 2>/dev/null") or "?"
	Host2 = mtkwifis.read_pipe("cat /tmp/mtk/host2 2>/dev/null") or "?"
	IP = mtkwifis.read_pipe("cat /tmp/mtk/ip 2>/dev/null") or "?"
	IP2 = mtkwifis.read_pipe("cat /tmp/mtk/ip2 2>/dev/null") or "?"
	os.execute("rm -rf /tmp/mtk/host /tmp/mtk/host2 /tmp/mtk/ip /tmp/mtk/ip2")
end
